class ItemsController < ApplicationController
  ...
  def approve
    ...
    @log = Log.new( :request_id => params[:request_id], :item_id => params[:item_id], :team_id => params[:team_id]);
    if Integer(params[:toApprove]) <= Integer(@item.item_remaining) and Integer(params[:toApprove]) <= @request.requested
      ...
      @log.requested = @requested.requested
      @log.approved = @requested.approved
